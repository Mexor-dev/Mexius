/// Minimal ZeroClaw foundation health check.
...
pub async fn initialize() -> Result<String, String> {
    ...
    // execution layer is embedded inside the goldclaw binary.
    Ok("internal-openclaw-embedded".to_string())
}

/// Initialize ZeroClaw toolset (Shell, File, Git) and report availability.
pub async fn init_tools() -> Result<Vec<(String, bool)>, String> {
    ...
    let file_ok = match tokio::fs::File::create("/tmp/goldclaw_file_test.tmp").await {
        Ok(_) => {
            let _ = tokio::fs::remove_file("/tmp/goldclaw_file_test.tmp").await;
            true
        }
        Err(_) => false,
    };
